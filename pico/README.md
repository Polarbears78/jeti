...
[Thonny](https://thonny.org/) (무료) 같은 편집기를 사용하면 쉽습니다.

1. Thonny 우하단에서 인터프리터를 **MicroPython (Raspberry Pi Pico)** 로 선택합니다.
2. `config.py`를 열어 **WiFi 이름/비밀번호**를 입력하고 저장합니다.
3. 이 폴더의 **세 파일을 모두 Pico에 저장**합니다 (Thonny: 파일 우클릭 → *Upload to /*):
   - `main.py`
   - `web.html`
   - `config.py`
...
2. Thonny **Shell 창**에 접속 주소가 출력됩니다:
   ```
   WiFi 연결 완료! 브라우저에서 접속:  http://192.168.0.42
   웹서버 시작. http://192.168.0.42 로 접속하세요.
   ```
3. **같은 WiFi**에 연결된 스마트폰/PC 브라우저에서 그 주소(`http://192.168.x.x`)로 접속합니다.
